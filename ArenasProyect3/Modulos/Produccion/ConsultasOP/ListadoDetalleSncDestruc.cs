...
namespace ArenasProyect3.Modulos.Produccion.ConsultasOP
{
    public partial class ListadoDetalleSncDestruc : Form
    {
        ...
        public void MostrarOrdenProduccion(DateTime fechaInicio, DateTime fechaTermino, string codigo = null)
        {
            using (SqlConnection con = new SqlConnection(Conexion.ConexionMaestra.conexion))
            using (SqlCommand cmd = new SqlCommand("OP_MostrarSNCList", con))
            {
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@fechaInicio", fechaInicio);
                    cmd.Parameters.AddWithValue("@fechaTermino", fechaTermino);
                    cmd.Parameters.AddWithValue("@codigoOp", (object)codigo ?? DBNull.Value);
                    try
                    {
                        con.Open();
        ...
        {
            MostrarOrdenProduccion(DesdeFecha.Value, HastaFecha.Value, txtBusqueda.Text);
        }
